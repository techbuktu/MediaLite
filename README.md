Objective 
-------------
I intended Mediaite to be a proof-of-concept newspaper outlet platform that enables its visitors to create User accounts and for its registered users to create Editor and/or Writer accounts.
...
* [MongoDB](): As the database of choice. The Mongoose package is used as an ODM (Object Data Model) and is used to make CRUD queries against the database.

* [ExpressJS](): Is the Node.js web framework used to build on top of the Node.js engine. It is currently perhaps the most feature-rich Node.js framework out there.

* [React.js](): Is arguably the most popular frontend/SPA framework in the web dev world at this point. I have also found it to be less-opinionated that Angular (my first love :) ) and as such has decided to select it for this project.

* [Redux](): I wanted to gauge the usefulness of this popular State Manager when integrated with a pseudo-complex project like MediaLite. So, instead of using the default React Context Api for State Management, I opted for Redux using the [react-redux]() package that connects the two technologies. I am impressed! :) 

* [Node.js](): Node.js is the engine on which Express and React run. Using NPM (its package manager and installer), one can install any needed third-party packages that are compatible and useful for the project at hand. In this case, I used NPM to install such packages as [mongoose](), [bcryptjs](), and others.

## Project Structure
Using my own opinionated "boilerplate" for how a full-stack app should the bootstrapped, I organized MediaLite to have three "entry" points. 

In my opinion, this makes it easy for the original developer (me, in this case) of a project to easily find re-locate specific portions of the codebase without *going crazy*.

The main "entry points" of the project are:

* ***Server.js***: This is the main entry point of the backend app. It is the "glue" that holds together the variouss routes files, connects the database and starts the project's server.

* ***backend/***: This folder contains the code for the backend (Express and co) of the project. Its subfolders are organized into exlicitly-named **models/**, **routes/**, **middleware/**, **views/**, **views/**, etc. 
...
1. I used [axios](), the Promise-based HTTP client for Node.js. It felt more intuitive and appeared more loaded (feature-wise) that the Fetch Api utility.
...
4. There's an intuitive HTTP verb-to-function mapping in how each method of the API services are named. Thus, UserApi.getUserByID() is clear about what it does and what args it should be fed, what type of data, it should get back from the server, etc. Compare this versus UserApi.getAllUsers() and UserApi.deleteUser(user_id). I wanted to each one to speak for itself and therefore, make some types of redundant documentation unnecessary (or, at least minimal, since the methods are acutely-"Englishmanic". :) )
...
1. *layout/*: Holds the components that determine the shared layout/UI of the app: Feader, Footer and Home. 
...
* **`dataStore/`**: This is where the all the logic of the Redux store resides. Again, opinionated as I am, I have decided to organize the "dataStore" into explicitly-named folders:

1. `actions/`: This has a `types/` sub-folder which contains one file each for the action TYPES related to each model. There is also a "_model_Actions.js" file (_e.g. `articleActions.js`, `writerActions.js`, etc.) for each model type. Each of these files contains the **action creators** (i.e functions) for the related model type.

2. `reducers/`: This contains a file each for each type of model of the frontend app. Thus, the `articleReducer.js` handles updateing the application state based on the *ACTION TYPE* that was dispatched from the *ACTION CREATORS* . The `rootReducer` itself is in the `reducers/index.js` file.

3. `index.js`: This is the "motherlode." This is where the app's Redux-based `store` is created and then exported (for use in `App.js`, etc.). The `rootReducer`, the frontend middleware (including `thunk`) also make "cameo" appearances here. This is also where the setup for the [REDUX DEV TOOLS]() is configured.
...
This should install all the packages that the backend app needs to run.
...
cd frontend
```
* Using the Node Package Manager (NPM) to install the dependencies:
```bash
npm install 
```
...
* This is a proof-of-concept project, is provided as-is with not guaranties whatsoever. If something goes awry, I didn't prescribe you the medicine! :)
...
* If your database needs are better cover by a different database (SQL or not), please, have a go at it, at your discretion.

### Authentication
* I used JWT Auth for this project. See [JWT.io](https://jwt.io) for details on its uses.
* If you prefer other token-based auth systems or even session-based auth, please, go ahead and try that. If it doesn't mesh well, you can always find your way back to the JWT world. It's an open world; you are always welcome! :) 
...
* See the Node.js/Express docs for the details on implementing your own custom middleware.
...
* You can greatly implement the UI appeal of this project (or others) using [reactstrap](), [Material Design Lite](https://getmdl.io) or other responsive web design frameworks. 
